...
export const generateShippingLabelPDF = async (order: Order): Promise<jsPDF> => {
  ...
  // Fix: Corrected payment method comparison
  const isCOD = order.payment_method === 'cod';
  
  // Fix: Added fallback for missing total_amount
  const totalAmount = Number(order.total_amount || order.total || 0);
  const subtotal = totalAmount / 1.18;
  const gstAmount = totalAmount - subtotal;
  
  // Fix: Added fallbacks for missing properties
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text(['Warehouse Node-A1', 'Industrial Logistics Estate', 'Support: 1800-DCH-HELP'], 20, 98);
  doc.setFontSize(8);
  ...
  order.items.forEach((item) => {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.text(`SKU: DCH-${item.id}`, 15, y + 4);
    y += 12;
  });
  ...
  doc.text(['ELECTRONIC FULFILLMENT MANIFEST', 'SUBJECT TO DAR CYCLE HUB JURISDICTION'], 45, footerY + 5);
  ...
  doc.text('DAR CYCLE HUB FULFILLMENT SYSTEM • MANIFEST V1.0 • SYSTEM GENERATED', 105, 285, { align: 'center', charSpace: 1 });
...
export const generateInvoicePDF = async (order: Order): Promise<jsPDF> => {
  ...
  // Fix: Added fallbacks for missing properties
  const primarySeller = order.seller_name || order.items[0]?.vendor_name || 'DAR CYCLE HUB';
  ...
  doc.text(['Warehouse Node-A1', 'Industrial Estate', 'GSTIN: 27DCHUB7788A1Z5'], 15, 50);
  ...
  doc.text('Generated by DAR CYCLE HUB Fulfillment Protocol', 105, 285, { align: 'center' });
