...
export const generateShippingLabelPDF = async (order: Order): Promise<jsPDF> => {
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text(['Warehouse Node-A1', 'Industrial Logistics Estate', 'Support: 1800-VXK-HELP'], 20, 98);
  doc.setFontSize(8);
  ...
  order.items.forEach((item) => {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.text(`SKU: VXK-${item.id}`, 15, y + 4);
    y += 12;
  });
  ...
  doc.text('VEXOKART FULFILLMENT SYSTEM • MANIFEST V1.0 • SYSTEM GENERATED', 105, 285, { align: 'center', charSpace: 1 });
...
export const generateInvoicePDF = async (order: Order): Promise<jsPDF> => {
  ...
  // Fix: Added fallbacks for missing properties
  const primarySeller = order.seller_name || order.items[0]?.vendor_name || 'VEXOKART';
  ...
  doc.text(['Warehouse Node-A1', 'Industrial Estate', 'GSTIN: 27VXKRT7788A1Z5'], 15, 50);
