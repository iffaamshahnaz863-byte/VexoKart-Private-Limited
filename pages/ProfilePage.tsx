    ...
    <div>
      ...
      <div className="p-4 space-y-6">
        <GlassmorphicCard className="p-6 flex items-center space-x-4">
          <img src="https://picsum.photos/seed/avatar/100" alt="User Avatar" className="w-20 h-20 rounded-full border-2 border-purple-500" />
          <div>
        ...
        {user.role === 'ADMIN' && (
            <Link to="/admin" className="block">
                <GlassmorphicCard className="p-4 flex items-center hover:bg-navy-light/70 transition">
        ...
        <GlassmorphicCard>
          {menuItems.map((item, index) => (
             <Link to={item.link} key={item.name} className={`flex items-center p-4 cursor-pointer hover:bg-navy-light/50 transition ${index !== menuItems.length - 1 ? 'border-b border-gray-700/50' : ''}`}>
        ...
        <button onClick={handleLogout} className="w-full bg-red-600/20 backdrop-blur-sm border border-red-500/50 text-red-400 font-bold py-3 rounded-xl hover:bg-red-600/40 transition">
          Logout
        </button>
      </div>
    </div>
