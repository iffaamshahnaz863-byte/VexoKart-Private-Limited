...
const ProductDetailPage: React.FC = () => {
  ...
  // If product is Daily Needs, it MUST be serviceable by LocationContext
  // If normal, we usually allow everywhere unless specifically restricted
  ...
  if (!product) return <div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-4 border-accent border-t-transparent rounded-full animate-spin"></div></div>;
  ...
  return (
    <div className="min-h-screen bg-surface pb-32">
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar py-2">
                ...
                <div className="shrink-0 flex flex-col items-center p-3 rounded-2xl border bg-orange-50 border-orange-100">
                    ...
                    <span className="text-[8px] font-black uppercase whitespace-nowrap text-accent">VexoShield Verified</span>
                </div>
            </div>
      ...
      {/* Sticky Action Bar */}
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-gray-100 z-[110] shadow-[0_-10px_30px_rgba(0,0,0,0.08)] flex gap-3">
            <button 
                onClick={handleAction.bind(null, 'cart')}
                disabled={!isServiceable}
                className="flex-1 h-14 bg-gray-900 text-white rounded-2xl font-black uppercase tracking-widest text-[11px] active:scale-95 transition-all disabled:opacity-50"
            >Add to Cart</button>
            <button 
                onClick={handleAction.bind(null, 'buy')}
                disabled={!isServiceable}
                className="flex-[1.5] h-14 bg-accent text-white rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-xl shadow-accent/20 active:scale-95 transition-all disabled:opacity-50"
            >
