...
const AdminLayout: React.FC = () => {
  ...
  const menuItems = [
    { name: 'Dashboard', path: '/admin', exact: true, icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
    ...
    { name: 'Approvals', path: '/admin/approvals', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z'},
    { name: 'Products', path: '/admin/products', icon: 'M4 6h16M4 12h16M4 18h7' },
    ...
    { name: 'Banners', path: '/admin/marketing', icon: 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.167H3.38a1.345 1.345 0 01-1.31-1.058l-.515-2.147a1.345 1.345 0 011.058-1.31h2.147l2.147-6.167a1.76 1.76 0 013.417.592zM17.25 12L21 8.25M17.25 12L21 15.75' },
    ...
    { name: 'Pincodes', path: '/admin/pincodes', icon: 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z' },
  ];
      ...
      <aside className="w-64 bg-white border-r border-gray-100 flex flex-col fixed h-full z-30 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        <div className="p-6 mb-2">
            <div className="flex items-center gap-2 mb-2">
                ...
                <h1 className="text-xl font-black text-gray-900 italic tracking-tighter uppercase">DAR CYCLE <span className="text-primary">HUB</span></h1>
            </div>
        ...
        <nav className="flex-grow px-3 space-y-0.5 overflow-y-auto no-scrollbar">
          {menuItems.map(item => {
            const isActive = location.pathname === item.path || (item.path === '/admin' && location.pathname.endsWith('/admin'));
            return (
              <NavLink
                key={item.name}
                to={item.path}
                end={item.exact}
                className={`flex items-center gap-3 py-3 px-4 rounded-2xl transition-all duration-200 group ${isActive ? 'bg-primary/5 text-primary shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
              >
                <svg className={`w-5 h-5 transition-colors ${isActive ? 'text-primary' : 'text-gray-300 group-hover:text-gray-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
